  ...
  return createPortal(
    <AnimatePresence>
      {open && (
        ...
        >
          {/* Backdrop — div not button (dismiss-area, not button
              activation). Any click outside the card dismisses. */}
          <div
            aria-hidden
            onClick={handleClose}
            className="absolute inset-0 bg-black/60 backdrop-blur-sm"
          />
          {/* Card. stopPropagation prevents inner clicks from leaking
              to the backdrop in any browser/event edge case. */}
          <motion.div
          ...
          >
            {/* Close X — top right. z-10 sits above any sibling
                content; explicit stopPropagation isolates the click. */}
            <button
              type="button"
              onClick={(e) => {
                e.stopPropagation();
...
function RoleToggle({
  ...
  return (
    <div
      role="radiogroup"
      aria-label="Are you a member or do you represent a gym?"
